...
const SectionProteinMaybe = props => {
  // Component's props should include all the possible options (from config)
  // and listing's publicData
  const { options, publicData } = props;
  const selectedOption =
    publicData && publicData.protein ? publicData.protein : null;

  // Don't return anything if public data doesn't contain protein field
  ...
  return (
    <div className={css.sectionFeatures}>
      <h2>
        <FormattedMessage
          id="ListingPage.proteinType"
          values={{ protein: optionLabel.toLowerCase() }}
        />
      </h2>
    </div>
  );
...
export default SectionProteinMaybe;